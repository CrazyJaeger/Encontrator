class Criatura {
    constructor(objeto, isFromJson = false) {
        ...
        if (isFromJson) {
            this.iniciativa = objeto.iniciativa;
            this.tiradaSalvacion = objeto.tiradaSalvacion;
            this.habilidades = objeto.habilidades;
            ...
            this.esConjurador = objeto.esConjurador;

            this.conjuros = [];
            for(const conj of objeto.conjuros){
                this.conjuros.push(new Conjuro(conj));
            }

            this.acciones = [];
            for(const acc of objeto.acciones){
                this.acciones.push(new Accion(acc));
            }

            this.accionesAdicionales = [];
            for(const acc of objeto.accionesAdicionales){
                this.accionesAdicionales.push(new Accion(acc));
            }

            this.reacciones = [];
            for(const acc of objeto.reacciones){
                this.reacciones.push(new Accion(acc));
            }
        } else {
    ...
    */
    ...
    toAccordion(idx){
        ...
        accordion.id = `combatiente_${idx}__atributos_accordion`;
        ...
        button.dataset.bsTarget = `#combatiente_${idx}__atributos_collapse`;
        button.dataset.ariaExpanded = "false";
        button.dataset.ariaControls = `combatiente_${idx}__atributos_collapse`;
        ...
        collapse.id = `combatiente_${idx}__atributos_collapse`;
        collapse.classList.add("accordion-collapse", "collapse");
        collapse.dataset.bsParent = `#combatiente_${idx}__atributos_accordion`;

        const body = document.createElement("div");
        body.classList.add("accordion-body");

        body.innerHTML = this.toMarkdown();
